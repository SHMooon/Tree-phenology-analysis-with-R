...
weather_Kiel<-handle_gsod(action="download_weather",
                     location=Kiel_list$chillR_code[11],
                     time_interval=c(1990,2020))
```
...
Kiel_patch_weather<-
      handle_gsod(action = "download_weather",
                  location = as.character(Kiel_list$chillR_code[c(9,14,24)]),
                  time_interval = c(1990,2020)) %>%
  handle_gsod()
...
```
cap the mean_bias at 1 °C and the stdev_bias at 2°C, and then look at the statistics again
...
There are two days missing after the patching. It seems safe to use linear interpolation for such a short gap. 
